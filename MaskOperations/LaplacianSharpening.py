...
def addFilter(image, filterImage, constant = 1):
    pixelMap = np.asarray(image, np.uint16) + np.asarray(filterImage, np.uint16)
...
def laplacianFilter(image):
    ...
    newPixelMap = applyOperator(pixelMap, operator)

    return Image.fromarray(newPixelMap.astype(np.uint8))


image = Image.open('goldengate_blur.jpg').convert('L')
sharpeningFilter = laplacianFilter(image)
sharpendImage = addFilter(image, sharpeningFilter, 2)
sharpeningFilter.show()
sharpendImage.show()